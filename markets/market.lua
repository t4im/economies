...
local function isOwner(player, pos) return player:get_player_name() == core.get_meta(pos):get_string("owner") end
...
markets.defaults = {
	...
	allow_metadata_inventory_put = function(pos, listname, index, stack, player)
		if not isOwner(player, pos) then
			...
			core.chat_send_player(player_name, "You are not allowed to configure another players market")
			economies.logAction("illegal inventory-put attempt by %s at %s", player_name, core.pos_to_string(pos))
			return 0
		end
		if listname == "item_type" then
			local virtual_stack = ItemStack(stack)
			virtual_stack:set_count(1)
			core.get_meta(pos):get_inventory():set_stack(listname, index, virtual_stack)
			return 0
		end
	...
	allow_metadata_inventory_take = function(pos, listname, index, stack, player)
		if not isOwner(player, pos) then
			local player_name = player:get_player_name()
			core.chat_send_player(player_name, "You are not allowed to configure another players market")
			economies.logAction("illegal inventory-take attempt by %s at %s", player_name, core.pos_to_string(pos))
			return 0
		end
		if listname == "item_type" then
	...
	on_metadata_inventory_put = function(pos, listname, index, stack, player)
		economies.logAction("inventory-put by %s at %s", player:get_player_name(), core.pos_to_string(pos))
	end,
	on_metadata_inventory_take = function(pos, listname, index, stack, player)
		economies.logAction("inventory-take by %s at %s", player:get_player_name(), core.pos_to_string(pos))
	end
}
